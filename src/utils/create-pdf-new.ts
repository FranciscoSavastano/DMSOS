...
doc.fontSize(linkFontSize)
   .text(`www.dmsys.com.br`, 440, 800, { link: 'http://dmsys.com.br', align: 'right', height: 50, width: 120}); // Set link behavior and underline
...
doc.fontSize(linkFontSize)
  ...
  const objectivetext = 'Este documento tem por objetivo apresentar, documentar e registrar todas as ocorrências, posturas e anomalias na operação diária do condomínio analisadas através do monitoramento das câmeras de vigilância (CFTV).'
  doc.fontSize(32).fill('#001233').text('OBJETIVO', { align: 'center' });
  doc.fontSize(15).fill('#001233').text(objectivetext,100, 200,{ lineGap: 10 });
